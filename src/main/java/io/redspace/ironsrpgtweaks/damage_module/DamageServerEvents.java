...
public class DamageServerEvents {
	...
	public static void onPlayerAttack(AttackEntityEvent event) {
		...
		if (event.getEntity().getAttackStrengthScale(0) < ServerConfigs.MINIMUM_ATTACK_STRENGTH.get()) {
			event.setCanceled(true);
		}
	}
	...
	private static boolean shouldProcess(DamageSource source, LivingEntity entityBeingAttacked) {
		if (ServerConfigs.DAMAGE_MODULE_ENABLED.get()) {
			return entityBeingAttacked instanceof Player ? ServerConfigs.PLAYER_DAMAGE_MODE.get() == PlayerDamageMode.NONE || (ServerConfigs.PLAYER_DAMAGE_MODE.get() == PlayerDamageMode.ONLY_LIVING && !(source.getDirectEntity() instanceof LivingEntity)) : true;
		}
		return false;
	}
